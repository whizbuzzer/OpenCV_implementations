...
image_path = currwd.replace("code/task_2/project_2a_task2_ANP_left_single.py", "images/task_2/")
# print(path)
...
limg0 = cv2.imread(image_path + "left_0.png")
limg1 = cv2.imread(image_path + "left_1.png")

# Getting the camera intrinsic parameters:
intrin_param_path = currwd.replace("code/task_2/project_2a_task2_ANP_left_single.py", "parameters/")
...
rms_err, cammat_l, distcoeff_l, cammat_r, distcoeff_r, R, T, E, F = cv2.stereoCalibrate(
    [objp_array[0]], [cornarray_l[0]], [cornarray_l[1]], KL, DL, KL, DL, (h_l, w_l), cv2.CALIB_FIX_INTRINSIC)
# print(retval)
'''
...
stereo_calib_params = cv2.FileStorage(currwd.replace("code/task_2/project_2a_task2_ANP_left_single.py", "parameters/stereo_calibration_l.xml"),
...
stereo_rect_params = cv2.FileStorage(currwd.replace("code/task_2/project_2a_task2_ANP_left_single.py", "parameters/stereo_rectification_l.xml"), 
    cv2.FileStorage_WRITE)
...
und_limg0 = cv2.undistortPoints(cornarray_l[0], KL, DL, pl, rl)
und_limg1 = cv2.undistortPoints(cornarray_l[1] , KL, DL, pl, rl)
...
P2 = np.hstack((R, T))
# print(P2)
triang_pts = cv2.triangulatePoints(P1, P2, und_limg0, und_limg1)
# print(triang_pts.shape)

# trix, triy, triz = triang_pts[0], triang_pts[1], triang_pts[2]
# plt.figure()
...
dist_l_0, dist_l_0_rect = getimg(limg0, KL, DL, rl, pl, (w_l, h_l))
dist_r_0, dist_r_0_rect = getimg(limg1, KL, DL, rl, pl, (w_r, h_r))

output_dir = currwd.replace("code/task_2/project_2a_task2_ANP_left_single.py", "output/task_2/left_single/")
chdir(output_dir)
...
cv2.imwrite("left_1_calibrated.png", dist_r_0)
cv2.waitKey(0)
cv2.imshow("calibration for right image 1 rectified", dist_r_0_rect)
cv2.imwrite("left_1_calibrated_rectified.png", dist_r_0_rect)
cv2.waitKey(0)
