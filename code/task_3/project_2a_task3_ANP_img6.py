...
image_path = currwd.replace("code/task_3/project_2a_task3_ANP_img6.py", "images/task_3_and_4/")
...
cam_param_path = currwd.replace("code/task_3/project_2a_task3_ANP_img6.py", "parameters/")
...
dist_l = cv2.remap(grey_l[6], mapx_l, mapy_l, cv2.INTER_LINEAR)

mapx_r, mapy_r = cv2.initUndistortRectifyMap(KR, DR, None, KR, (w_r, h_r), 5, cv2.CV_16SC2)  # right_0.png
dist_r = cv2.remap(grey_r[6], mapx_r, mapy_r, cv2.INTER_LINEAR)
...
output_dir = currwd.replace("code/task_3/project_2a_task3_ANP_img6.py", "output/task_3/")
img_l = cv2.drawKeypoints(dist_l, keypoints_l, None, color=(255, 0, 0), flags = 0)
plt.imshow(img_l)
cv2.imwrite(output_dir+"left_6_features.png", img_l)
plt.show()
img_r = cv2.drawKeypoints(dist_r, keypoints_r, None, color=(0, 0, 255), flags = 0)
cv2.imwrite(output_dir+"right_6_features.png", img_r)
plt.imshow(img_r)
plt.show()
...
cv2.imwrite(output_dir+"matches_drawn_6.png", desc_match_drawn)
plt.show()
